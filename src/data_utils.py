...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    ...
    for line in set(raw_kb_triples + keep_triples + removed_triples):
        ...
        entity_hist[e1] += 1
        entity_hist[e2] += 1
        # entity_hist[e1] = entity2id[e1] + 2
        # entity_hist[e2] = entity2id[e2] + 2
        ...
        relation_hist[r] += 1
        # relation_hist[r] = relation2id[r] + 3

        if add_reverse_relations:
            inv_r = r + '_inv'

            if inv_r not in relation_hist.keys():
                relation_hist[inv_r] += 1
                # relation_hist[inv_r] = relation_length
                # relation_length += 1

    use_attention = True
    if use_attention:
        entity2id = read_entity_from_id(os.path.join(data_dir, 'entity2id.txt'))
        relation2id = read_relation_from_id(os.path.join(data_dir, 'relation2id.txt'))
        relation_length = len(relation2id) + 3
        entity_hist = collections.defaultdict(int)
        relation_hist = collections.defaultdict(int)

        for e, e_id in entity2id.items():
            entity_hist[e] = e_id + 2

        for r, r_id in relation2id.items():
            relation_hist[r] = r_id + 3
            if add_reverse_relations:
                inv_r = r + '_inv'
                relation_hist[inv_r] = relation_length
                relation_length += 1
