...
from src.emb.fact_network import get_conve_nn_state_dict, get_conve_kg_state_dict, \
    get_complex_kg_state_dict, get_distmult_kg_state_dict, get_attention_nn_state_dict, get_attention_kg_state_dict
...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        if fn_model in ['conve']:
            ...
            fn_state_dict = torch.load(args.conve_state_dict_path)
            fn_kg_state_dict = get_conve_kg_state_dict(fn_state_dict)
        elif fn_model == "attention":
            use_conve_emb = False
            fn_state_dict = torch.load(args.attention_state_dict_path)
            fn_nn_state_dict = get_attention_nn_state_dict(fn_state_dict)
            if use_conve_emb:
                fn_conve_state_dict = torch.load(args.conve_state_dict_path)
                fn_kg_state_dict = get_conve_kg_state_dict(fn_conve_state_dict)
            else:
                fn_kg_state_dict = get_attention_kg_state_dict(fn_state_dict)
            self.fn.load_attention_state_dict(fn_nn_state_dict)
        else:
